...
/*
	...
	Author: Bryan "Tonic" Boardwine, modded by Index
...
[0,"STR_NOTF_BeingSeized",true,[SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
...
if(EQUAL(count crew _vehicle,0)) then {
	...
	hint format[localize "STR_NOTF_Seized",_type,_price];
	[0,"STR_NOTF_HasSeized",true,[profileName,SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
	ADD(BANK,_price);
} else {
